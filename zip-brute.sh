...
loading_bar() {
  ...
  # use printf "%b" so escape sequences inside variables are interpreted
  ...
  for percent in $(seq 0 1 100); do
    ...
    # print with %b so ${CYAN} etc. are interpreted
...
crack_zip() {
  ...
  while IFS= read -r password || [ -n "$password" ]; do
    ...
    # Build the colored bar parts and print using %b so escape codes render
    left="${CYAN}["
    middle="${filled_str}${empty_str}"
    right="${CYAN}]"
    # print: colored bar (as %b), percentage (as %3d), and colored trying message (as %b)
...
build_combined_wordlist_from_folder() {
  ...
  if [ ! -d "$folder" ]; then
    printf "%b\n" "${RED}[!] Wordlists folder not found: $folder${NC}"
    return 1
  fi
  ...
  if [ ${#files[@]} -eq 0 ]; then
    printf "%b\n" "${RED}[!] No .txt wordlist files found in $folder${NC}"
    return 1
  fi
  ...
  if [ ! -s "$tmp_combined" ]; then
    rm -f "$tmp_combined"
    printf "%b\n" "${RED}[!] Combined wordlist is empty after processing.${NC}"
    return 1
  fi

  printf "%b\n" "$tmp_combined" >/dev/null 2>&1
  echo "$tmp_combined"
  return 0
}
...
main() {
  ...
  case "$opt" in
    1)
      ...
      ;;
    2)
      tmp_combined_path=$(build_combined_wordlist_from_folder)
      if [ $? -ne 0 ]; then
        printf "%b\n" "${RED}[!] Could not build combined wordlist. Make sure ./wordlists exists and contains .txt files.${NC}"
        exit 1
      fi
      crack_zip "$zip_file" "$tmp_combined_path"
      rm -f "$tmp_combined_path"
      ;;
    *)
